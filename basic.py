...
class ParseResult:
    ...
    def try_register(self, res):
        if res.error:
            self.to_reversed_count = res.advance_count
            return None
...
class Parser:
    ...
    def statements(self):
        ...
        while True:
            ...
            if not statement:
                self.reverse(res.to_reversed_count)
                more_statements = False
                continue
            statements.append(statement)
    ...
    def bin_op(self, func_a, ops, func_b=None):
        ...
        while True:
            op_type = self.current_tok.type
            op_value = self.current_tok.value

            matched = False

            for op in ops:
                if isinstance(op, tuple):
                    if op_type == op[0] and op_value == op[1]:
                        matched = True
                        break
                else:
                    if op_type == op:
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_pop(self, exec_ctx):
        ...
        try:
            ...
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                'Element at this index could not be removed from list because is out of bounds.',
                exec_ctx
            ))
